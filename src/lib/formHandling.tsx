...
interface Validators<Values> {
  [fieldName: string]: Validator<Values>
}

export interface InnerFormProps<Values> {
  errors: { [key in keyof Values]: ErrorValidationResult }
  formValues: Values
  setFormValue: (fieldName: string, newValue: string) => any
  validate: (formValues: Values) => boolean
}

export function addFormState<Values, Props = {}>(
  options: { defaultValues?: Values; validators?: Validators<Values> } = {}
) {
  ...
  return compose<Props & InnerFormProps<Values>, Props>(
    ...
    withHandlers<
      {
        formValues: Values
        setErrors: (errors: ErrorStateUpdater) => any
        setFormValues: (values: Values) => any
      },
      {}
    >({
